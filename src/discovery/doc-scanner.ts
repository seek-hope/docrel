...
function parseFile(absPath: string, projectRoot: string): ParsedDocSection[] | null {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException)?.code !== 'ENOENT') {
      console.warn(`DocRelay: parseFile failed for ${absPath}:`, (err as NodeJS.ErrnoException)?.message ?? err);
    }
    return null;
  } finally {
    if (fd !== undefined) {
...
/** Collect all doc files recursively. Skips hidden dirs, common non-doc dirs, and .docrelayignore patterns. */
function collectDocFiles(dir: string, projectRoot: string): string[] {
  ...
  while (stack.length > 0 && result.length < MAX_FILES) {
    ...
    try {
      ...
      console.warn(`DocRelay: cannot read directory ${path.relative(projectRoot, current)}:`, err instanceof Error ? err.message : err);
      continue;
    }
